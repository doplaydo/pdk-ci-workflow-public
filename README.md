...
| `release.yml` | prepare, build-wheels, build-doc-pdf, release-private/pypi, github-release | Full release pipeline on green CI: tag at the CI-tested SHA, wheels, PDF docs, registry publish, GitHub Release |
...
| `ANTHROPIC_API_KEY` | claude-pr-review, cut-release (changelog generation) |
...
|---------|---------------|
| `check-required-files` | README.md, CHANGELOG.md, LICENSE, Makefile, pyproject.toml, .gitignore, .pre-commit-config.yaml, tests/, test workflow |
...
- `ANTHROPIC_API_KEY` - Claude code reviews and changelog generation (claude-pr-review, cut-release)
