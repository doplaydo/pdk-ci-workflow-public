...
| `check-precommit-config` | `check_precommit_config.py` | `.pre-commit-config.yaml` includes required hooks (end-of-file-fixer, trailing-whitespace, ruff or ruff-lint, ruff-format, pydocstyle) and recommended hooks (nbstripout, codespell) |
| `check-template-drift` | `check_template_drift.py` | Enforces `.github/dependabot.yml` and `.github/workflows/*.yml` thin callers match upstream `templates/`. Ruff-style auto-fix: missing or drifted files are rewritten from the canonical template, exit 1; re-run exits 0. Conditionally enforces `sample-projects.yml` only in repos containing a `*--sample-projects/` directory. Deletes deprecated templates (listed in `DEPRECATED_TEMPLATES`) if still present. No-ops inside pdk-ci-workflow itself. |
